...
<div style="max-width: 100%;">
    <div style="background: white; border-radius: 15px; padding: 30px; box-shadow: var(--shadow1);">
        <div style="display: flex; align-items: center; margin-bottom: 30px;">
            ...
            <h1 style="margin: 0; color: var(--black); font-size: 1.8rem;">Edit Buku</h1>
        </div>
        ...
        <form action="{{ route('admin.books.update', $book->id) }}" method="POST" enctype="multipart/form-data" id="bookForm">
                ...
                <div>
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <select name="format" required
                            ...
                            <option value="print" {{ old('format', $book->format) === 'print' ? 'selected' : '' }}>Cetak</option>
                        </select>
                    </div>
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <div style="position: relative;">
                            <input type="text" id="categorySearch" placeholder="Cari atau pilih kategori..." autocomplete="off"
                                   value="{{ old('category_search', $book->category ? ($book->category->name ?? $book->category) : '') }}"
                                   style="width: 100%; padding: 12px; border: 2px solid #e1e5e9; border-radius: 8px; font-size: 1rem; transition: border-color 0.3s ease;"
                            ...
                            <input type="hidden" name="category_id" id="categoryId" value="{{ old('category_id', $book->category_id) }}">
                            <div id="categoryDropdown" style="display: none; position: absolute; top: 100%; left: 0; right: 0; background: white; border: 2px solid #e1e5e9; border-top: none; border-radius: 0 0 8px 8px; max-height: 200px; overflow-y: auto; z-index: 1000; box-shadow: 0 4px 6px rgba(0,0,0,0.1);">
                                @foreach($categories as $category)
                                    <div class="category-option" data-id="{{ $category->id }}" data-name="{{ $category->name }}" 
                                         ...
                                         style="padding: 10px 12px; cursor: pointer; border-bottom: 1px solid #f0f0f0; transition: background-color 0.2s; {{ $book->category_id == $category->id ? 'background-color: #e8f5e8;' : '' }}"
                                         onmouseover="this.style.backgroundColor='#f8f9fa'" 
                                         onmouseout="this.style.backgroundColor='{{ $book->category_id == $category->id ? '#e8f5e8' : 'white' }}'">
                                        {{ $category->name }}
                                    </div>
                                @endforeach
                ...
                <div>
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <textarea name="description" placeholder="Masukkan deskripsi buku" rows="4"
                                  ...
                                  onfocus="this.style.borderColor='var(--green1)'" onblur="this.style.borderColor='#e1e5e9'">{{ old('description', $book->description) }}</textarea>
                    </div>
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <textarea name="author_info" placeholder="Masukkan informasi tentang penulis" rows="3"
                                  style="width: 100%; padding: 12px; border: 2px solid #e1e5e9; border-radius: 8px; font-size: 1rem; transition: border-color 0.3s ease; resize: vertical;"
                                  onfocus="this.style.borderColor='var(--green1)'" onblur="this.style.borderColor='#e1e5e9'">{{ old('author_info', $book->author_info) }}</textarea>
                    </div>
            ...
            <div style="margin-top: 30px; padding-top: 30px; border-top: 2px solid #f0f0f0;">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 30px; align-items: start;">
                    <div>
                        <label style="display: block; margin-bottom: 8px; font-weight: 500; color: var(--black);">Upload Gambar Cover Baru</label>
                        <div style="border: 2px dashed #e1e5e9; border-radius: 8px; padding: 20px; text-align: center; transition: all 0.3s ease;" 
                            ...
                            <p style="margin: 10px 0 0; font-size: 0.85rem; color: var(--green1); font-weight: 500;">Kosongkan jika tidak ingin mengubah gambar</p>
                        </div>
                    </div>
                    
                    <div>
                        <label style="display: block; margin-bottom: 8px; font-weight: 500; color: var(--black);">Preview</label>
                        <div id="imagePreview" style="width: 100%; height: 200px; border: 2px solid #e1e5e9; border-radius: 8px; display: flex; align-items: center; justify-content: center; background: #f8f9fa; overflow: hidden;">
                            @if($book->cover_image)
                                ...
                                <div id="noImagePlaceholder" style="display: none; text-align: center; color: #999; flex-direction: column; align-items: center; justify-content: center;">
                                    <i class='bx bx-image' style="font-size: 3rem; margin-bottom: 10px;"></i>
                                    <p style="margin: 0;">Gambar tidak ditemukan</p>
                                </div>
                            @else
                                <div style="text-align: center; color: #999;">
                                    <i class='bx bx-image' style="font-size: 3rem; margin-bottom: 10px;"></i>
                                    <p style="margin: 0;">Belum ada gambar cover</p>
                                </div>
                            @endif
                        </div>
                    </div>
